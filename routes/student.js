...
import create_report  from '../report/createReport'
...
export default function(io, app, passport, queue)
{
	...
	const llvmDir = __dirname + "/../../cse231-proj1/"
	...
	io.on('connection', function(socket)
	{
		...
		socket.on('analyzeCode', code =>
		{
			console.log(code)
			/*fs.writeFile(llvmDir + "extra/benchmarks/student/student.cpp", code.userCode, function(err) 
			{
				if(err) return console.log(err);
				console.log("The file was saved!");	
			})  // file is now saved, let the analysis begin!*/
			child_process.execFile(llvmDir + "init.sh").on('close', data =>
			{
				fs.readFile(llvmDir + "outputss/log.txt", "utf8", (err1, data1) => 
				{
					...
					for(var i = 0; i < lines.length; i++)
					{
						...
						if(lines[i].startsWith("Test Case "))
						{
							...
							if(!(test_cases.find(x => x['TestCase'] == test_number)))
								test_cases.push({'TestCase': test_number, 'result': 'Passed'})
						}
						...
						if (lines[i] == "Failed")
						{
							...
							test_cases.find(x => x['TestCase'] == test_number)['result'] = 'Failed'
						}
					...
					//console.log(results)
					socket.emit('analyzeCode', { results, test_cases})
				})
			})
		})
	})
