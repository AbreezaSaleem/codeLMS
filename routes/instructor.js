...
export default function(app, passport)
{
	...
	router.post('/loginI/instructor/course/new', async ctx =>	// do the gitlab-project shit here!!!
	{
		...
		console.log(ctx.request.body.students)
		let students_enrolled = ctx.request.body.students
		console.log(students_enrolled)
		for (var i = 0; i < students_enrolled.length; i++) 
		{
			console.log('code ' + ctx.request.body.course)
			gradecomponent_creator(students_enrolled[i], ctx.request.body.course) // umm... so here, it sort of works for one student. not the whole freaking list
		}
		ctx.body = { studentList: 'studentList' }
	})
