		...
		value: function componentDidMount() {
			...
			this.setState({
				...
				_this2.props.getStudentsEnrolled({ courseCode: _this2.state.courseCode }).then(function (response) {
					_this2.setState({
						studentsEnrolled: response.data.studentList.students_enrolled.map(function (student) {
							return student.split('|')[1];
						})
					});
					console.log('students enrolled are!!!');
					console.log(_this2.state.studentsEnrolled);
				});
				_this2.setState( // uhhh u gotta do the spilliting here! fak
				{
					studentsEnrolled: "18100114"
				});
			});
		}
	}, {
